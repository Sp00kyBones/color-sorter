...
void setup() {
  ...
  // Declare motor step/direction pins + LED as output
>>>>>>> parent of 1419683 (reformats vscode formatter)
...
<<<<<<< HEAD
    ...
    if (tcs.begin()) {
        ...
        std::exit(0);  // I think this is what you mean -> exit program because
                       // there is no color sensor (right?)
    }
...
void loop() {
    switch (Serial.read()) {
        case 'Y':
            ...
            delay(1000);
            Serial.print("[DEBUG] Motors energized \n");
            delay(3000);
            Serial.print("[DEBUG] Commencing 'MOTOR TEST' sequence");
            delay(3000);
            break;
        case 'N':
            // example case
    ...
    // Spooky - not sure if you wanted the for loops in your switch case logic,
    // but I've added the default
    //          case and closed the conditional suite. You can move your logic
    //          back in the approperiate cases.

    // TODO: pull these motor actions into their own class
    // motor test sequence:
    for (int x = 0; x <= 4; x++) {  // test sequence consists of 4 cycles
        ...
        for (int i = 0; i < stepsPerRevolution * 2; i++) {
            ...
            delayMicroseconds(4000);  // how fast the motor will turn, as value
                                      // increases motor rotates slower
...
>>>>>>> parent of 1419683 (reformats vscode formatter)
    ...
    delay(5000);
    Serial.println(
        "After you fill the hopper, ensure that the enclosure is closed and "
        "enter Y to start sorting the balls. \t***press N to stop the "
        "process***");
    running = false;
<<<<<<< HEAD
}
